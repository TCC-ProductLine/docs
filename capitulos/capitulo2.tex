...
Se todas as aplicações usassem a mesma interface de programação e protocolo de interoperabilidade todo o trabalho de software seria mais simples. Essa é a premissa que trouxe o desenvolvimento orientado a serviços para o mundo da tecnologia da informação, e qando se desenvolve dessa forma os serviços tornam-se a base para a criação de novas estratégias de solução \cite{Newcomer:Lomow:2004}.
...
Um ``\textit{Web Service}'' é uma noção abstrata que deve ser implementada por uma agente concreto, este agente é um pedaço de software que envia e recebe mensagens. O serviço é o recurso caracterizado pela funcionalidade abstrata que é servida \footnote{http://www.w3.org/TR/ws-arch/}.

O XML hoje é amplamente utilizado quase como o protocolo HTTP \footnote{http://www.w3.org/Protocols/HTTP/1.1/rfc2616bis/draft-lafon-rfc2616bis-03.html} e faz parte da solução para aplicação de ``\textit{Web Services}'', é usado para realizar a formatação dos dados \cite{Alonso:Casati:Kuno:Machiraju:2010}.

Simplificando, os ``\textit{Web Services}'' possuem algumas três características e tecnologias principais que são usadas para desenvolvê-los, o XML que é a especificação sobre a qual estes são construídos, WSDL\footnote{http://www.w3.org/TR/wsdl} que provêem as interfaces fundamentais que serão usadas e SOAP\footnote{http://www.w3.org/TR/soap/} que é responsável pela comunicação com outros serviços. Pode-se dizer que um ``\textit{Web Service}'' deve fornecer interfaces de uso simples e se comunicar facilmente com outros serviços \cite{Newcomer:Lomow:2004}.
...
REST é um estilo arquitetural para sistemas hipermídia distribuídos, reúne um grupo de critérios que serão incorporados ao projeto de aplicações distribuídas. Os Serviços RESTful são serviços web que seguem os critérios REST \cite{Filho:2009}.

Os serviços da web vem a cada dia buscando soluções mais simples e mais leves para compor suas funcionalidades. Os serviços RESTFul introduzem um novo tipo de abstração que busca oferecer as funcionalidades já existentes dos serviços da web comum, porém, de uma forma mais leve e simples \cite{Pautasso:2009}.
...
\begin{itemize}
	...
	\item \textbf{Escopo de Execução:} Neste caso o serviço RESTFul também se utiliza do protocolo HTTP, porém, diz respeito aos parâmetros que devem ser enviados junto da requisição para que o serviço possa tratar o recurso corretamente, pode ser a identificação de um objeto em um conjunto de recursos, por exemplo.
\end{itemize}
